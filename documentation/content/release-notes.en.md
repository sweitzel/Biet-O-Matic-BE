...
- Added displaying the browser storage usage in the footer of the BE overview page.
- Default bidding-time changed from 5s to 10s to workaround late bids reported by some users.
- Removed automated adjustment of maximum bid to the minimum price increase.
  The item maximum bid has now to be manually entered by the user.
- Fixed wrong international number formatting on item page.
...
- Complete rewrite of bidding process
...
- Add info messages for adding items from watch list.
...
- Extended event logging:
  - BE now saves changes to article parameters in addition to bidding events.
    For example, a Log created if the item price has been updated or the maximum bid has been changed by the user.
- Adjustment of the handling of items which end close to each other:
  ...
  However, it still has to be tested whether this is sufficient or could be too much.
